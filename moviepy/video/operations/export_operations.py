...
class ExportOperations:
    ...
    def save_frame(self, filename, t=0, with_mask=True):
        ...
        if with_mask and self.video_clip.mask:
            mask = (255 * self.video_clip.mask.get_frame(t)).astype(np.uint8)
            frame = np.dstack([frame, mask])
        imsave(filename, frame.astype(np.uint8))
    ...
    def write_videofile(self, filename, fps=None, codec=None, bitrate=None, audio=True,
        ...
        if make_audio:
            audiofile = temp_audiofile or os.path.join(temp_audiofile_path,
                                                       f"{name}_TEMP_AUDIO.{find_extension(audio_codec)}")

        ffmpeg_write_video(self.video_clip, filename, fps, codec, bitrate, preset, write_logfile,
                           audiofile, threads, ffmpeg_params, logger, pixel_format)
    ...
    def write_images_sequence(self, name_format, fps=None, with_mask=True, logger="bar"):
        ...
        """
        timings = np.arange(0, self.video_clip.duration, 1.0 / fps)
        filenames = [name_format % i for i, t in enumerate(timings)]
        for name, t in zip(filenames, timings):
            self.save_frame(name, t, with_mask)
        return filenames
    ...
    def write_gif(self, filename, fps=None, program="imageio", opt="nq", fuzz=1,
        ...
        """
        if program == "imageio":
            write_gif_with_image_io(self.video_clip, filename, fps, opt, loop, colors, logger)
        elif tempfiles:
            write_gif_with_tempfiles(self.video_clip, filename, fps, program, opt, fuzz,
                                     loop, dispose, colors, logger, pixel_format)
        else:
            write_gif(self.video_clip, filename, fps, program, opt, fuzz, loop, dispose,
                      colors, logger, pixel_format)
